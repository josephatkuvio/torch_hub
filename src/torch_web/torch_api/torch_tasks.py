...
from .models import Task, Parameter
...
def torch_task(name, description=None):
    def decorate(func):
        ...
        parameters = [Parameter(name=k,value=str(v.default) if v.default is not inspect.Parameter.empty else None) for k, v in spec.parameters.items() if k != 'specimen' ]
